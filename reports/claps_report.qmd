  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        % Left-align tables (APA), so each aligns flush left with its caption and note.
        \setlength{\LTleft}{0pt}
        \setlength{\LTright}{\fill}
        % knitr::kable adds \addlinespace every fifth row to group rows, which shows
        % as an uneven gap before the last row of a six-row table. Zero it out.
        \AtBeginDocument{\setlength{\defaultaddspace}{0pt}}
execute:
  echo: false
  warning: false
  message: false
...
The analysis code, configuration and this report are openly available at
<https://github.com/pablobernabeu/CLAPS-analysis>. The report rebuilds from the
committed summary tables, so it needs no raw data, which are held private to CLAPS
until the project reaches a more advanced stage.
...
supported by adult grammaticality-judgement and comprehension studies
...
Developmental and processing evidence converges on the same constraint. Verb
classes differ in how strongly they support passive comprehension and production:
children acquire actional, highly affected-patient passives earlier and more
reliably than non-actional ones, and affectedness-like semantic factors remain
explanatory even when syntactic form is controlled [@maratsosFoxBeckerChalkley1985;
...
Hebrew and Mandarin. The affectedness constraint on passive acceptability has not yet
been tested in Turkish or Norwegian. CLAPS therefore extends the paradigm
to two typologically and morphologically distinct languages for which no
language-specific effect-size estimates yet exist. The focal-slope priors
accordingly draw on cross-language rather than language-specific values (see below).
...
The cumulative-logit model suits these data because it respects the
discreteness, ordering and bounded nature of Likert ratings while leaving the
latent scale free to vary continuously across conditions [@burknerVuorre2019].
Treating the same ratings as continuous, under a Gaussian likelihood, biases
regression coefficients and inflates both Type I and Type II error where floor
or ceiling effects are present [@liddellKruschke2018].
...
The `primary` regime is zero-centred and weakly-to-moderately informative, the `weak`
regime relaxes all focal-slope scales, the `literature_centred` regime is a
direction-encoding sensitivity check, and the `heavy_tailed` regime substitutes
Student-t for Gaussian focal-slope priors.
...
towards the identity matrix and so regularises the many correlation parameters of the
...
across sample size, their sensitivity to the prior, the resulting per-language
sample-size recommendation and the refinements planned before the design is finalised.
...
and the secondary metric is $BF_{10} > `r BF_THRESHOLD_SECONDARY`$ (moderate evidence),
grades that follow @jeffreys1961. Bayes factors are
...
n_cells <- 30800L; n_fail <- 0L
if (file.exists(fail_path)) {
  fd <- readr::read_csv(fail_path, show_col_types = FALSE)
  n_cells <- fd$total_cells[1]
  n_fail  <- sum(fd$n_error[1], fd$n_timeout[1], fd$n_oom[1], fd$n_malformed[1],
                 na.rm = TRUE)
}
fail_clause <- if (n_fail == 0) {
  "None failed through estimation error, timeout, exhaustion of memory or malformed output"
} else {
  sprintf("Of these, %s failed through estimation error, timeout, exhaustion of memory or malformed output",
          format(n_fail, big.mark = ","))
}
```

Every one of the `r format(n_cells, big.mark = ",")` design cells that was launched
produced a usable fit. `r fail_clause`. The planned models are therefore feasible
across the full grid of sample sizes, prior regimes and model-ladder levels.
...
every focal hypothesis. @fig-power shows the power curves for the two focal
hypotheses under the primary prior. The full operating characteristics across all
conditions, prior regimes and hypotheses are reported in @tbl-exceedance.
...
if (file.exists(exc_path)) {
  ...
  plot_df <- exc_df |>
    ...
    dplyr::mutate(
      ...
      # 95% Jeffreys credible interval for the exceedance proportion: the Monte-Carlo
      # uncertainty over the n_sims replicates, well behaved at the ceiling.
      n_succ = round(p_bf_primary * n_sims),
      ci_lo  = stats::qbeta(0.025, n_succ + 0.5, n_sims - n_succ + 0.5),
      ci_hi  = stats::qbeta(0.975, n_succ + 0.5, n_sims - n_succ + 0.5)
    )
  pc_labels <- plot_df |>
  ...
  ggplot2::ggplot(plot_df,
    ...
    ggplot2::geom_errorbar(ggplot2::aes(ymin = ci_lo, ymax = ci_hi),
                           width = 2.5, linewidth = 0.4, alpha = 0.55) +
    ggplot2::geom_line(linewidth = 0.7) +
    ggplot2::geom_point(size = 1.8) +
    ggplot2::geom_label(data = pc_labels,
                        ggplot2::aes(x = 71, y = y_lab, label = Hypothesis),
...
*Note.* Points are exceedance probabilities, *P*(*BF*~10~ > 10), under the primary prior, with error bars showing 95% credible intervals (Jeffreys) for the Monte-Carlo estimate. The dashed line marks the conventional 80% target. The pooled cross-language analysis was simulated to *N* = 60 and at lower replication, so its intervals are wider.

## Prior Sensitivity of BF Category

Prior choice shifts the Bayes-factor power only modestly. @tbl-priorsens reports the
...
```{r}
#| label: tbl-priorsens
#| tbl-cap: "Prior Sensitivity of Statistical Power, as a Percentage, Across Prior Regimes at N = 50 (Maximal Model, 20 Verbs)"
...
if (file.exists(sens_path)) {
  ...
  sens_tbl <- sens_exc |>
    ...
    dplyr::rename_with(~ "LC", dplyr::any_of("literature_centred")) |>
    dplyr::arrange(Language, Hypothesis)
  knitr::kable(sens_tbl, booktabs = TRUE) |>
...
*Note.* LC = literature-centred.

## Model Ladder Selection Frequency

The ladder rarely needed to step down. @tbl-ladder records how often each level was
the highest feasible model across the simulated fits.

```{r}
#| label: tbl-ladder
#| tbl-cap: "Frequency With Which Each Model-Ladder Level Was the Highest Feasible Model"
...
if (file.exists(ladder_path)) {
  ...
  knitr::kable(ladder_df, booktabs = TRUE)
} else {
...
met the publication-grade convergence criteria set out in Part I. @tbl-maximal gives
it for each language alongside the number of converged cells.

```{r}
#| label: tbl-maximal
#| tbl-cap: "Maximal Feasible (Converged) Model Level per Language"
...
if (file.exists(mfm_path)) {
  ...
  knitr::kable(mfm_df, booktabs = TRUE)
} else {
...
much heavier than the single-language ones. @tbl-runtime summarises per-cell runtimes
and convergence rates.

```{r}
#| label: tbl-runtime
#| tbl-cap: "Per-Cell Fit Runtimes (Minutes) and Convergence Rate, by Language and Model Level"
rt_path <- file.path(SUM_DIR, "runtime_summary.csv")
if (file.exists(rt_path)) {
  rt_df <- readr::read_csv(rt_path, show_col_types = FALSE)
  knitr::kable(rt_df, booktabs = TRUE, digits = 2) |>
    kableExtra::kable_styling(font_size = 9)
} else {
...
and gender conditions, with a Monte-Carlo *SE* of about `r sprintf("%.3f", mc_se)` near a true rate
of 0.8, and `r n_sims_cross` for cross-language conditions. The recommended sample sizes are
therefore stable long-run rates rather than the indicative 0/1 values of the earlier
single-seed feasibility run (archived under `outputs/design_analysis_feasibility_v1`,
which also documents the L5 maximal-model convergence and per-fit runtimes). All
...
warrants caution. The relevant literature is subject to publication bias and to the
structural incentives of academic publishing, which together tend to inflate
reported effects. Studies that report positive findings, especially small and
low-powered ones, on average overstate the magnitude of what they detect, the
...
The analysis reported here is an early, decision-relevant pass, and several refinements
are planned before the design is finalised. The most consequential concerns the number
of simulated datasets per design point. Bayes-factor design analysis is conventionally
run with on the order of 10,000 Monte-Carlo replicates per point
[@schonbrodtWagenmakers2018], against the `r n_sims_single` used here for the
single-language and gender conditions and the `r n_sims_cross` for the cross-language
conditions. Near a power of 0.80, that increase would tighten the Monte-Carlo standard
error of each estimate from approximately `r sprintf("%.3f", mc_se)` to approximately
`r sprintf("%.3f", sqrt(0.8 * 0.2 / 10000))`. The single-language and gender designs use
comparatively light fits and can approach this standard directly. The cross-language
design, at 8 to 12 hours per fit, cannot, and would instead be raised towards 1,000
replicates as the compute budget allows.

Three further refinements are planned. The pooled model would be re-run at the
correlated-maximal random-effects structure used for the single languages rather than
the uncorrelated structure adopted here for feasibility, placing every language on the
same modelling footing. The data-generating effect sizes would be varied across a
plausible range, from the literature anchor down to the conservative 'safeguard' value
noted above, so that the recommendation is reported as a range rather than a single
point. The design grid would also be made denser, with finer sample-size spacing,
several cross-language sample sizes in place of the single point used here, participants
and items varied jointly, and the prior-sensitivity regimes carried across the full
sample-size range rather than the single anchor. None of these is expected to overturn
the headline recommendation. They would sharpen the precision of the estimates and test
...
@tbl-exceedance reports the probability of exceeding each Bayes-factor threshold for
every design condition.

```{r}
#| label: tbl-exceedance
#| tbl-cap: "Probability of Exceeding BF > 10 (Primary) and BF > 3 (Secondary) per Design Condition"
exc_path <- file.path(SUM_DIR, "bf_exceedance.csv")
if (file.exists(exc_path)) {
  exc_df <- readr::read_csv(exc_path, show_col_types = FALSE)
  exc_tbl <- exc_df |>
...
*Note.* Hyp denotes the focal hypothesis, Sims the number of simulated replicates and LC the literature-centred regime. P(BF > 10) and P(BF > 3) are the proportions of replicates exceeding each Bayes-factor threshold.
