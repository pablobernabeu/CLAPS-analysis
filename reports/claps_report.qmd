  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \usepackage{caption}
        \DeclareCaptionFormat{apa7}{\textbf{#1}\\[5pt]\textbf{#3}}
        \DeclareCaptionFormat{apa7par}{\textbf{#1}\par\textbf{#3}}
        \captionsetup{format=apa7, labelsep=none, justification=raggedright,
          singlelinecheck=false, skip=6pt}
        \captionsetup[longtable]{format=apa7par, labelsep=none,
...
The primary `proposal` prior set therefore uses zero-centred Gaussians concentrated
...
the `proposal` regime is zero-centred and weakly-to-moderately informative; `weak`
...
change across the four prior regimes (proposal, weak, literature-centred and
...
The pilot analysis fixes the inputs to the design analysis. The `proposal` prior is
...
hypotheses under the proposal prior. The full operating characteristics across all
...
if (file.exists(exc_path)) {
  ...
  plot_df <- exc_df |>
    dplyr::filter(prior_regime == "proposal", n_verbs == 20,
...
*Note.* Points are exceedance probabilities, *P*(*BF*~10~ > 10), under the proposal prior. The dashed line marks the conventional 80% target. The pooled cross-language analysis was simulated to *N* = 60.
...
if (file.exists(sens_path)) {
  ...
  # The non-proposal regimes were simulated at a single anchor sample size, so
  ...
  sens_tbl <- sens_exc |>
    ...
    dplyr::relocate(dplyr::any_of("proposal"), .after = Hypothesis) |>
