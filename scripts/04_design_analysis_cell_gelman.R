#!/usr/bin/env Rscript
# scripts/04_design_analysis_cell_gelman.R
# Prior-scale-corrected variant of scripts/04_design_analysis_cell.R: identical
# in every respect except that it sources R/06_simulate_design_gelman.R, which
# overrides the simulators so the ANALYSED focal predictor is Gelman-scaled
# (SD 0.5, as in the real pipeline) instead of the raw U(-0.5,0.5) draw (SD 0.289).
# See R/06_simulate_design_gelman.R for the rationale. Run a single cell from a
# grid row index, as a SLURM array task.
# Usage: Rscript scripts/04_design_analysis_cell_gelman.R [--row_index N] [--overwrite]
...
source("R/06_simulate_design_gelman.R")   # <- Gelman-scaled predictor override
source("R/07_extract_diagnostics.R")

option_list <- list(
  optparse::make_option("--row_index", default = NULL, type = "integer",
    help = "Row in the grid (1-based). Defaults to SLURM_ARRAY_TASK_ID."),
  optparse::make_option("--grid",      default = "config/design_grid_corrected_scale.csv"),
  optparse::make_option("--config",    default = "config/analysis_config.yaml"),
  optparse::make_option("--outdir",    default = "outputs/design_corrected_scale"),
