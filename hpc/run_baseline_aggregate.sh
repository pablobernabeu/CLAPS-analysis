...
# N = 30-100, proposal prior, maximal model), excluding the gender variation and
# the cross-language analysis BY SEED RANGE. The proposal/maximal cells are
...
#   single prior-sensitivity    : seeds 200000-201799 (N 50, non-proposal regimes)
...
# the proposal filter) plus 400000-402399 (extension). The focal table then
# filters prior_regime==proposal, model_level==L5_correlated_maximal, n_verbs==20.
set -euo pipefail
...
echo "== baseline focal table (proposal / maximal / 20 verbs) =="
Rscript -e '
...
pc <- exc %>% filter(model_level=="L5_correlated_maximal", prior_regime=="proposal",
